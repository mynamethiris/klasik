...
async function recompressExistingImages() {
  try {
    ...
    for (const file of files) {
      ...
      if (stat.size < 150 * 1024) continue; // skip files already <150KB
  ...
  } catch (e) { console.error('recompressExistingImages error:', e); }
}
...
async function startServer() {
  ...
  app.get("/api/server-time", (req, res) => { const now = new Date(); res.json({ time: now.toISOString(), date: now.toISOString().split('T')[0] }); });
  ...
  app.post("/api/shuffle-members", (req, res) => {
    ...
    // Respect day blacklist
    const blacklistSetting = (db.prepare("SELECT value FROM settings WHERE key = 'schedule_day_blacklist'").get() as any)?.value || '[]';
    let blacklist: string[] = [];
    try { blacklist = JSON.parse(blacklistSetting); } catch { blacklist = []; }
    const availableDays = DAYS_ORDER.filter(d => !blacklist.includes(d));

    const shuffledMembers = shuffle(allMembers);
    const shuffledDays = shuffle([...availableDays]).slice(0, pjCount);
  ...
  app.post("/api/schedules/randomize-names", (req, res) => {
    const schedules = db.prepare("SELECT group_name FROM schedules").all() as any[];
    if (schedules.length === 0) return res.json({ success: true, message: "Belum ada jadwal" });

    const usedTerms: string[] = [];
    const doUpdate = db.transaction(() => {
      for (const s of schedules) {
        const oldName = s.group_name;
        let newTerm = "";
        const available = COMPUTER_TERMS.filter(t => !usedTerms.includes(t));
        if (available.length > 0) {
          newTerm = available[Math.floor(Math.random() * available.length)];
        } else {
          newTerm = `${COMPUTER_TERMS[Math.floor(Math.random() * COMPUTER_TERMS.length)]} ${usedTerms.length + 1}`;
        }
        ...
        db.prepare("UPDATE users SET group_name = ? WHERE group_name = ?").run(newName, oldName);
      }
    });
  ...
  // Promo links CRUD
  app.get("/api/promo-links", (req, res) => {
    const links = db.prepare("SELECT * FROM promo_links ORDER BY sort_order ASC, id ASC").all();
    res.json(links);
  });

  app.post("/api/promo-links", (req, res) => {
    const { type, label, url, sort_order } = req.body;
    if (!type || !label || !url) return res.status(400).json({ success: false, message: "Data tidak lengkap" });
    db.prepare("INSERT INTO promo_links (type, label, url, sort_order) VALUES (?, ?, ?, ?)").run(type, label, url, sort_order || 0);
    res.json({ success: true });
  });

  app.put("/api/promo-links/:id", (req, res) => {
    const { type, label, url, sort_order } = req.body;
    db.prepare("UPDATE promo_links SET type = ?, label = ?, url = ?, sort_order = ? WHERE id = ?").run(type, label, url, sort_order || 0, req.params.id);
    res.json({ success: true });
  });

  app.delete("/api/promo-links/:id", (req, res) => {
    db.prepare("DELETE FROM promo_links WHERE id = ?").run(req.params.id);
    res.json({ success: true });
  });
  ...
  // PJ: edit absent members (with time limit check)
  app.post("/api/report/:report_id/edit-absents", (req, res) => {
    const { report_id } = req.params;
    const { absentMembers, description } = req.body;
    const report = db.prepare("SELECT *, submitted_at FROM reports WHERE id = ?").get(report_id) as any;
    if (!report) return res.status(404).json({ success: false, message: "Laporan tidak ditemukan" });
    const settings = db.prepare("SELECT value FROM settings WHERE key = 'edit_time_limit_minutes'").get() as any;
    const limitMin = parseInt(settings?.value || '15');
    ...
    const diff = (Date.now() - submitted.getTime()) / 60000;
    if (diff > limitMin) return res.status(403).json({ success: false, message: `Batas waktu edit (${limitMin} menit) telah terlewati` });
    db.prepare("DELETE FROM absent_members WHERE report_id = ?").run(report_id);
    db.prepare("UPDATE reports SET cleaning_description = ? WHERE id = ?").run(description, report_id);
    if (Array.isArray(absentMembers)) {
      const ins = db.prepare("INSERT INTO absent_members (report_id, member_id, name, reason) VALUES (?, ?, ?, ?)");
      const today = report.date;
      const pj_id = report.pj_id;
      const VIOLATION_STATUSES: Record<string, string> = {
        'Alfa': 'Alfa', 'Sakit (Tanpa Surat)': 'Sakit (Tanpa Surat)',
        'Izin Telat': 'Telat', 'Telat': 'Telat', 'Tidak Piket': 'Tidak Piket',
      };
      for (const m of absentMembers) {
        ins.run(report_id, m.member_id || null, m.name, m.reason);
        const violationType = VIOLATION_STATUSES[m.reason];
        if (violationType) {
          const existing = db.prepare("SELECT id FROM violations WHERE member_name = ? AND date = ? AND type = ?").get(m.name, today, violationType);
          if (!existing) db.prepare("INSERT INTO violations (member_id, member_name, pj_id, date, type) VALUES (?, ?, ?, ?, ?)").run(m.member_id || null, m.name, pj_id, today, violationType);
        }
      }
    }
    res.json({ success: true });
  ...
  app.post("/api/admin/report/:report_id/edit-photo", upload.single("photo"), async (req, res) => {
    const { report_id } = req.params;
    const { photoType } = req.body;
    const report = db.prepare("SELECT id FROM reports WHERE id = ?").get(report_id) as any;
    if (!report) return res.status(404).json({ success: false, message: "Laporan tidak ditemukan" });
    if (!req.file) return res.status(400).json({ success: false, message: "Tidak ada foto" });
    let photoPath = "";
    try { photoPath = await compressImage(req.file.buffer, req.file.originalname); } catch { return res.status(500).json({ success: false, message: "Gagal kompresi foto" }); }
  ...
  // ADMIN: edit absent members (no time limit)
  app.post("/api/admin/report/:report_id/edit-absents", (req, res) => {
    const { report_id } = req.params;
    const { absentMembers, description } = req.body;
    const report = db.prepare("SELECT *, date, pj_id FROM reports WHERE id = ?").get(report_id) as any;
    if (!report) return res.status(404).json({ success: false, message: "Laporan tidak ditemukan" });
    db.prepare("DELETE FROM absent_members WHERE report_id = ?").run(report_id);
    db.prepare("UPDATE reports SET cleaning_description = ? WHERE id = ?").run(description, report_id);
    if (Array.isArray(absentMembers)) {
      ...
      // Clear old violations for this date/report members, then re-insert
      db.prepare("DELETE FROM violations WHERE date = ? AND pj_id = ?").run(today, pj_id);
      for (const m of absentMembers) {
        ins.run(report_id, m.member_id || null, m.name, m.reason);
        const violationType = VIOLATION_STATUSES[m.reason];
        if (violationType) {
  ...
  // ADMIN: Recompress existing images
  app.post("/api/admin/recompress-images", async (req, res) => {
    recompressExistingImages().then(() => { }).catch(() => { });
    res.json({ success: true, message: "Kompresi ulang dimulai di background" });
  });

  // Random group name from COMPUTER_TERMS
  app.get("/api/random-group-name", (req, res) => {
    const usedNames = (db.prepare("SELECT group_name FROM schedules").all() as any[]).map((r: any) => r.group_name);
    const available = COMPUTER_TERMS.filter(t => !usedNames.some(n => n.includes(t)));
    const pool = available.length > 0 ? available : COMPUTER_TERMS;
    const term = pool[Math.floor(Math.random() * pool.length)];
    res.json({ group_name: `Kelompok ${term}` });
  });
  ...
  app.delete("/api/reports/:id", (req, res) => {
    try {
      ...
      if (report) {
        // Cascade delete violations for this report (based on date and pj_id)
        db.prepare("DELETE FROM violations WHERE date = ? AND pj_id = ?").run(report.date, report.pj_id);
      }
  ...
  app.post("/api/reports/reset", (req, res) => {
    ...
    db.prepare("DELETE FROM admin_notifications WHERE type = 'report'").run();
    db.prepare("DELETE FROM violations").run();
    res.json({ success: true });
  });

  app.post("/api/violations/reset", (req, res) => {
    db.prepare("DELETE FROM violations").run();
    res.json({ success: true });
  });
  ...
  app.listen(PORT, "0.0.0.0", () => {
    ...
    recompressExistingImages().catch(() => { });
  });
}
