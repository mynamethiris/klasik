...
async function startServer() {
  ...
  app.get("/api/substitutions", (req, res) => {
    ...
    const subs = pj_id
      ? db.prepare(`${base} WHERE s.requester_pj_id = ? OR s.substitute_pj_id = ? ORDER BY s.created_at DESC`).all(pj_id, pj_id)
      : db.prepare(`${base} ORDER BY s.created_at DESC`).all();
    res.json(subs);
  });
  ...
  app.post("/api/substitutions", (req, res) => {
    ...
    const debtCount = db.prepare("SELECT COUNT(*) as c FROM substitutions WHERE requester_pj_id = ? AND status = 'pending'").get(requester_pj_id) as any;
    if (debtCount.c >= 3) {
      return res.status(400).json({ success: false, message: "Batas hutang jadwal (3) tercapai. Selesaikan hutang terlebih dahulu." });
    }
    db.prepare("INSERT INTO substitutions (requester_pj_id, substitute_pj_id, original_date, substitute_date, status) VALUES (?, ?, ?, ?, 'pending')").run(requester_pj_id, substitute_pj_id, original_date, substitute_date || null);
    res.json({ success: true });
  });
