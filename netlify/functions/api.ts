...
export const handler: Handler = async (event: HandlerEvent, _ctx: HandlerContext) => {
  ...
  try {
    ...
    if (method === "DELETE" && segments[0] === "users" && segments[1] && !segments[2]) {
      const id = segments[1];
      // Get group_name before delete for schedule cascade
    ...
    if (method === "DELETE" && segments[0] === "members" && segments[1]) {
      const id = segments[1];
      // Auto-delete PJ user if this member is a PJ
    ...
    // Add notification on attendance
    if (method === "POST" && segments[0] === "attendance") {
      ...
      await db.execute({ sql: "INSERT INTO admin_notifications (type, message, pj_id, pj_name) VALUES (?, ?, ?, ?)", args: ['attendance', `PJ ${pjName} mengirim absensi (${fields.status}) pukul ${fields.time} WIB`, fields.pj_id, pjName] });
      return json(200, { success: true });
    }

    // Add notification on report
    ...
    if (method === "GET" && segments[0] === "notifications" && !segments[1]) {
      const r = await db.execute("SELECT * FROM admin_notifications ORDER BY created_at DESC LIMIT 50");
      return json(200, r.rows);
    }
    ...
    if (method === "DELETE" && segments[0] === "notifications" && segments[1]) {
    ...
    if (method === "POST" && segments[0] === "shuffle-members") {
      ...
      for (let i = 0; i < pjCount; i++) {
        ...
        for (const m of groupMembers) {
          await db.execute({ sql: "UPDATE class_members SET pj_id = ? WHERE id = ?", args: [newPJId, m.id as number] });
        }
        await db.execute({ sql: "UPDATE users SET member_id = ? WHERE id = ?", args: [pjMember.id as number, newPJId] });
        if (shuffledDays[i]) {
          await db.execute({ sql: "INSERT INTO schedules (group_name, day) VALUES (?, ?)", args: [groupName, shuffledDays[i]] });
        }
      }
      return json(200, { success: true });
    }

    // POST /schedules/import-text
    if (method === "POST" && segments[0] === "schedules" && segments[1] === "import-text") {
      ...
      for (const g of valid) {
        ...
        if (existSched.rows.length > 0) {
          await db.execute({ sql: "UPDATE schedules SET group_name = ? WHERE id = ?", args: [groupName, existSched.rows[0].id as number] });
        } else {
          await db.execute({ sql: "INSERT INTO schedules (group_name, day) VALUES (?, ?)", args: [groupName, g.day] });
        }
        ...
        if (pjMemberRes.rows.length === 0) {
          await db.execute({ sql: "INSERT INTO class_members (name, pj_id) VALUES (?, ?)", args: [g.pjName, pjId] });
        } else {
          await db.execute({ sql: "UPDATE class_members SET pj_id = ? WHERE id = ?", args: [pjId, pjMemberRes.rows[0].id as number] });
        }
        ...
        for (const mName of g.memberNames) {
          ...
          if (existingM.rows.length === 0) {
            await db.execute({ sql: "INSERT INTO class_members (name, pj_id) VALUES (?, ?)", args: [mName, pjId] });
          } else {
            await db.execute({ sql: "UPDATE class_members SET pj_id = ? WHERE id = ?", args: [pjId, existingM.rows[0].id as number] });
          }
        }
      }
